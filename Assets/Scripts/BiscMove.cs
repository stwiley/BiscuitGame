    ...
    void FixedUpdate()
    {
        if(active) // movement is relative to the camera, make sure "CamFollow" is enabled before trying to move.
        {
            ...
            if (Input.GetKey(KeyCode.D))
            {
                ...
                movevec += new Vector3(tmpvec.x, 0, tmpvec.z);
                torquevec += -cam.transform.forward;
            }
            ...
            GetComponent<Rigidbody>().AddForce(Vector3.Normalize(movevec) * Mathf.Max(transform.lossyScale.x, transform.lossyScale.y, transform.lossyScale.z));
            GetComponent<Rigidbody>().AddTorque(Vector3.Normalize(torquevec) * Mathf.Max(transform.lossyScale.x, transform.lossyScale.y, transform.lossyScale.z));
        }
    }
